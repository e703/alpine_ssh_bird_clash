...
curl -L -o routes4.conf https://github.com/e703/nchnroutes-k/releases/download/v1.0.0/routes4.conf
# https://github.com/haotianlPM/nchnroutes-k/releases/download/v1.0.0/routes4.conf

# 下载 routes6.conf 文件
echo "下载 routes6.conf 文件..."
curl -L -o routes6.conf https://github.com/e703/nchnroutes-k/releases/download/v1.0.0/routes6.conf
...
if [ "$filesize_routes4_kb" -gt "400" ]; then
  echo "移动 routes4.conf 文件到 /etc/bird 文件夹..."
#  cp -f routes4.conf /etc/bird/routes4.conf
mv routes4.conf /etc/bird/routes4.conf

#  echo "移动 routes6.conf 文件到 /etc/bird 文件夹..."
